...
/* On iOS, the C library function `system()` is unavailable.  The MINC
 * volume_io helpers use `system()` to invoke external commands, which is
 * appropriate for desktop/CLI tools but not for sandboxed iOS apps.
 * To allow ITK's internal MINC library to build on iOS, we route all
 * uses of `system()` through a macro that becomes a no-op on iOS.
 */
#if defined(__APPLE__) && defined(TARGET_OS_IPHONE) && TARGET_OS_IPHONE
#  define VIO_SYSTEM(cmd) (-1)
#else
#  define VIO_SYSTEM(cmd) system(cmd)
#endif
    ...
    FILE               **file )
{
    ...
    if( gzipped )
    {
        ...
        command_status = VIO_SYSTEM( command );
