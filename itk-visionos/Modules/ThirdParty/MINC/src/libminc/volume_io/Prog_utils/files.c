...
@METHOD     : Portable buffered copy (no system()) for iOS/visionOS/macOS.
@GLOBALS    : 
@CALLS      :  
@CREATED    : Feb.  3, 1997    David MacDonald
@MODIFIED   : 2025-12          (Apple/visionOS-safe implementation)
---------------------------------------------------------------------------- */
    ...
    VIO_STR  dest )
{
    ...
    VIO_STR      src_expanded, dest_expanded;
    FILE        *in_file = NULL;
    FILE        *out_file = NULL;
    ...
    in_file = fopen( src_expanded, "rb" );
    if( in_file == NULL )
    {
        print_error( "Error opening source file %s for copy.  ", src_expanded );
        print_system_error();
        status = VIO_ERROR;
        goto cleanup;
    }

    out_file = fopen( dest_expanded, "wb" );
    if( out_file == NULL )
    {
        print_error( "Error opening destination file %s for copy.  ", dest_expanded );
        print_system_error();
        status = VIO_ERROR;
        goto cleanup;
    }

    while( (n_read = fread( buffer, 1, sizeof(buffer), in_file )) > 0 )
    {
        if( fwrite( buffer, 1, n_read, out_file ) != n_read )
        {
            print_error( "Error writing to %s while copying file.  ", dest_expanded );
            print_system_error();
            status = VIO_ERROR;
            break;
        }
    ...
    if( ferror( in_file ) )
    {
        print_error( "Error reading from %s while copying file.  ", src_expanded );
        print_system_error();
        status = VIO_ERROR;
    }

cleanup:
    if( in_file  != NULL ) (void) fclose( in_file );
    if( out_file != NULL ) (void) fclose( out_file );
...
@METHOD     : Uses rename(), falling back to copy+unlink (no system()).
@GLOBALS    : 
@CALLS      :  
@CREATED    : Feb.  3, 1997    David MacDonald
@MODIFIED   : 2025-12          (Apple/visionOS-safe implementation)
    ...
    VIO_STR  dest )
{
    ...
    /* First try a simple rename (works on same filesystem). */
    if( rename( src_expanded, dest_expanded ) != 0 )
    {
        /* Fall back to copy+unlink. */
        status = copy_file( src, dest );
        if( status == VIO_OK )
        {
            if( unlink( src_expanded ) != 0 )
            {
                print_error( "Error removing original file %s after move.  ",
                             src_expanded );
                print_system_error();
                status = VIO_ERROR;
            }
...
@CALLS      : 
@CREATED    :                      David MacDonald
@MODIFIED   : 2025-12 (no system() for gunzip/bunzip2 on Apple platforms)
---------------------------------------------------------------------------- */
    ...
    FILE               **file )
{
    ...
    /* --- On Apple sandboxed platforms, do NOT shell out to gunzip/bunzip2.
           Instead, fail clearly if a compressed file is requested. */

    if( gzipped )
    {
        print_error(
          "Error: compressed files are not supported by open_file() on this "
          "platform for file \"%s\".\n", expanded );
        status = VIO_ERROR;
    }
