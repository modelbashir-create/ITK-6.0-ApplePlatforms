...
//: cpp traits!
#define vnl_T_n_aux_1(T)  \
  (T x) { data_[0] = x; } \
  static_assert(true, "")
#define vnl_T_n_aux_2(T) \
  (T x, T y)             \
  {                      \
    data_[0] = x;        \
    data_[1] = y;        \
...
#define vnl_T_n_class_impl(T, n)                      \
  ...
  public:                                             \
    vnl_##T##_##n() = default;                        \
    vnl_##T##_##n(vnl_vector<T> const & rhs)          \
      : vnl_vector_fixed<T, n>(rhs)                   \
    {}                                                \
    vnl_##T##_##n(vnl_vector_fixed<T, n> const & rhs) \
      : vnl_vector_fixed<T, n>(rhs)                   \
    {}                                                \
