...
main(int, char *[])
{
  ...
  {
    std::cout << "MET_SYSTEM_BYTE_ORDER_MSB = TRUE" << '\n';
  }
  else
  {
    std::cout << "MET_SYSTEM_BYTE_ORDER_MSB = FALSE" << '\n';
  }
  unsigned short x = 256;
  std::cout << "MET_ByteSwapShort: ";
  if (MET_ByteOrderSwapShort(x) != 1)
  {
    std::cout << "FAILED: 256->" << MET_ByteOrderSwapShort(x) << '\n';
  ...
  {
    std::cout << "FAILED: 1->" << MET_ByteOrderSwapShort(x) << '\n';
    exitCode = EXIT_FAILURE;
  }
  else {
    std::cout << "PASSED" << '\n';
  ...
  if ((*fieldIter)->defined)
  {
    ...
    if (nDims != 2)
    {
      std::cout << "nDims not equal to 2" << '\n';
      exitCode = EXIT_FAILURE;
    }
    else {
      std::cout << "nDims: Passed" << '\n';
}
  }
  else
  {
    std::cout << "nDims not defined" << '\n';
    exitCode = EXIT_FAILURE;
  }
  ...
  if ((*fieldIter)->defined)
  {
    ...
    {
      std::cout << "ElementSizes are wrong: " << eSize[0] << ", " << eSize[1] << '\n';
      exitCode = EXIT_FAILURE;
    }
    else {
      std::cout << "ElementSizes: Passed" << '\n';
}
  }
  else
  {
    std::cout << "ElementSize not defined" << '\n';
    exitCode = EXIT_FAILURE;
  }
  ...
  if ((*fieldIter)->defined)
  {
    ...
    if (nNames != 2) {
      std::cout << "nNames wrong : " << nNames << '\n';
    } else if (strcmp(names[0], "X-AXIS") != 0 || strcmp(names[1], "Y-AXIS") != 0)
    {
      std::cout << "names wrong : _" << names[0] << "_, _" << names[1] << "_" << '\n';
      exitCode = EXIT_FAILURE;
    }
    else {
      std::cout << "Names: Passed" << '\n';
}
  }
  else
  {
    std::cout << "DirNames not defined" << '\n';
    exitCode = EXIT_FAILURE;
  }
